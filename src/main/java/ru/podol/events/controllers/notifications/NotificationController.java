...
public class NotificationController {
    ...
    @GetMapping
    public ResponseEntity<List<Notification>> getNotificationsByUserId(@AuthenticationPrincipal UserDto userDto) {
        if (isUserDtoIsNull(userDto)) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(Collections.emptyList());
        }

        List<Notification> notifications = notificationService.getNotificationsByUserId(userDto.getId());
